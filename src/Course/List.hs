...
filter ::
  ...
  -> List a
  -> List a
filter f as = foldRight (\a as -> if f a then a :. as else as) Nil as
...
(++) ::
  ...
  -> List a
  -> List a
(++) as bs = foldRight (\a as -> a :. as) bs as

-- point free notation : (++) =  flip (foldRight (:.))
...
find ::
  ...
  -> List a
  -> Optional a
find p = foldRight (\a ls -> if p a then Full a else ls) Empty 
...
reverse ::
  List a
  -> List a
reverse = foldLeft (\a bs -> bs :. a) Nil
