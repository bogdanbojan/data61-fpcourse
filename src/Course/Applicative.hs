...
instance Applicative ExactlyOne where
  ...
  (<*>) eoab eoa = 
    let f = runExactlyOne eoab
        a = runExactlyOne eoa
    in ExactlyOne (f a) -- you can also do it with mapExactlyOne; it'll be shorter
...
instance Applicative List where
  ...
  (<*>) ::
    ...
    -> List a
    -> List b
  (<*>) f a  = flatMap (\f' -> map f' a) f -- flatMap (flip fmap a) f
...
lift2 ::
  ...
  -> k b
  -> k c
lift2 f ka kb = pure f <*> ka <*> kb -- f <$> ka <*> kb => (f <$> ka <*>) pointfree  
...
lift3 ::
  ...
  -> k c
  -> k d
lift3 f ka kb kc =  pure f <*> ka <*> kb <*> kc -- f <$> ka <*> kb <*> kc
...
lift1 ::
  ...
  -> k a
  -> k b 
lift1 f ka = pure f <*> ka -- can be reduced to (pure f <*>)
...
(<*) ::
  ...
  -> k a
  -> k b
(<*) kb ka = lift2 (\kb _ -> kb) kb ka -- lift 2 (const) fa fb <=> lift 2 const   
                                       -- you can use (flip const) to the *> operator problem above
...
replicateA ::
  ...
  -> k (List a)
replicateA n ka = 
  let x = replicate n ka  -- sequence (replicate n ka)
  in sequence x
...
filtering ::
  ...
  -> k (List a)
filtering p xs =
 foldRight (\a kxs -> lift2 (\b xs' -> if b then a :. xs' else xs') (p a) kxs) (pure Nil) xs
